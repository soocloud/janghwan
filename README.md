...
**카드 이미지 한 장만 올리면 페이지가 자동으로 만들어집니다.** 글은 쓰고 싶을 때만 덧붙이면 됩니다.

### 방법 A — 이미지만 올리기 (기본)

`src/assets/history/` 폴더에 아래 규칙으로 이미지를 넣습니다.

```
2026-08-20 대한제국 선포.jpg     ← 날짜 + 제목
2026-08-20.jpg                    ← 날짜만 (제목은 "2026년 8월 20일의 역사")
```

| 규칙   | 내용                                                                            |
| ------ | ------------------------------------------------------------------------------- |
| 날짜   | 파일 이름 **맨 앞**에 `2026-08-20` (`20260820`, `2026.08.20`, `260820` 도 인식) |
| 제목   | 날짜 뒤에 **한 칸 띄고** 적습니다. 생략 가능                                    |
| 확장자 | `.jpg` `.jpeg` `.png` `.webp` `.avif`                                           |

이것만으로 아래가 전부 자동 처리됩니다.

- `/history/2026-08-20/` 페이지 생성
- 달력의 해당 날짜에 표시 · 클릭 가능
- 목록·검색 색인·RSS·카카오톡 공유 미리보기 등록

> 날짜로 시작하지 않는 파일(예: `unnamed.jpg`)은 **무시**되고 빌드 로그에 경고가 남습니다.

**아버님이 직접 올리시는 방법 (GitHub 웹)**

1. 아래 주소를 즐겨찾기에 등록해 두고 클릭
   `https://github.com/soocloud/janghwan/upload/main/src/assets/history`
2. 이미지 파일을 창 안으로 끌어다 놓기
3. 맨 아래 초록색 **Commit changes** 클릭

2~3분 뒤 사이트에 자동으로 반영됩니다. (아버님 GitHub 계정을 저장소 Collaborator 로 초대해 두어야 합니다.)

### 방법 B — 이미지 + 본문 글

이미지 아래에 설명 글까지 붙이고 싶을 때만 씁니다.
`src/content/history/_템플릿.md` 를 복사해 **날짜 이름**으로 저장합니다.
...
같은 날짜의 이미지가 **자동으로 짝지어지므로 `image` 항목은 적지 않아도 됩니다.**

| 항목       | 설명                                                      |
| ---------- | --------------------------------------------------------- |
| `date`     | **필수.** 달력 기준 날짜. 파일 이름과 똑같이 맞춰 주세요. |
| `title`    | **필수.** 카드 제목 (이미지 파일명의 제목보다 우선합니다) |
